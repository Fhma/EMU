...
public class ValidationBoolean {
	...
	public void setUp() throws Exception {
		...
		String m_path = ValidationBoolean.class.getResource("resources/04model.xmi").getPath();
		String mm_path = ValidationBoolean.class.getResource("resources/01Metamodel.ecore").getPath();
		IModel model = createEmfModel("M", m_path, mm_path, true, false);
		module.getContext().getModelRepository().addModel(model);
	}

	@Test
	public void primitive_types_boolean_valid() throws Exception {
		module.parse(getFile("primitive_types_boolean_valid"));
		module.execute();
	...
	@Test
	public void primitive_types_boolean_invalid_replace_equal_value() throws Exception {
		module.parse(getFile("primitive_types_boolean_invalid_replace_equal_value"));
		module.execute();
		assertEquals(1, getMatrixSize(module.getOperatorsMatrix().getContent().values()));
		Set<Map.Entry<String, List<String>>> operators_map = module.getOperatorsMatrix().getContent().entrySet();
		Iterator<Map.Entry<String, List<String>>> it = operators_map.iterator();
	...
	@Test
	public void primitive_types_boolean_invalid_incompatible() throws Exception {
		module.parse(getFile("primitive_types_boolean_invalid_incompatible"));
		module.execute();
		assertEquals(1, getMatrixSize(module.getOperatorsMatrix().getContent().values()));
		Set<Map.Entry<String, List<String>>> operators_map = module.getOperatorsMatrix().getContent().entrySet();
		Iterator<Map.Entry<String, List<String>>> it = operators_map.iterator();
	...
	private File getFile(String s) {
		return new File(ValidationBoolean.class.getResource("resources/" + s + ".emu").getPath());
	}
}
