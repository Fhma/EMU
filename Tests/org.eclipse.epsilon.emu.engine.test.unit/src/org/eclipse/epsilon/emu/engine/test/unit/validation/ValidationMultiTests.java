 ...
 * @since 2017
 */
public class ValidationMultiTests {
	private EmuModule module;

	@Before
	public void setUp() throws Exception {
		module = new EmuModule();
		String m_path = ValidationMultiTests.class.getResource("resources/04model.xmi").getPath();
		String mm_path = ValidationMultiTests.class.getResource("resources/01Metamodel.ecore").getPath();
		IModel model = createEmfModel("M", m_path, mm_path, true, false);
		module.getContext().getModelRepository().addModel(model);
	}

	@Test
	public void multi_valids() throws Exception {
		module.parse(getFile("multi_valids"));
		module.execute();
		assertEquals(6, getMatrixSize(module.getOperatorsMatrix().getContent().values()));
	...
	@Test
	public void multi_invalids() throws Exception {
		module.parse(getFile("multi_invalids"));
		module.execute();
		assertEquals(10, getMatrixSize(module.getOperatorsMatrix().getContent().values()));
		Set<Map.Entry<String, List<String>>> operators_map = module.getOperatorsMatrix().getContent().entrySet();
		Iterator<Map.Entry<String, List<String>>> it = operators_map.iterator();
		while (it.hasNext()) {
			for (String s : it.next().getValue())
	...
	private File getFile(String s) {
		return new File(ValidationMultiTests.class.getResource("resources/" + s + ".emu").getPath());
	}
}
