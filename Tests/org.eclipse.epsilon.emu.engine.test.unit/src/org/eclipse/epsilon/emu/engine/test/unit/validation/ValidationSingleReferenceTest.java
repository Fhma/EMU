 ...
 * @since 2017
 */
public class ValidationSingleReferenceTest {
	private EmuModule module;

	@Before
	public void setUp() throws Exception {
		module = new EmuModule();
		String m_path = ValidationSingleReferenceTest.class.getResource("resources/04model.xmi").getPath();
		String mm_path = ValidationSingleReferenceTest.class.getResource("resources/01Metamodel.ecore").getPath();
		IModel model = createEmfModel("M", m_path, mm_path, true, false);
		module.getContext().getModelRepository().addModel(model);
	}

	@Test
	public void single_reference_valids() throws Exception {
		module.parse(getFile("single_reference_valids"));
		module.execute();
		assertEquals(4, getMatrixSize(module.getOperatorsMatrix().getContent().values()));
	...
	@Test
	public void single_reference_invalids() throws Exception {
		module.parse(getFile("single_reference_invalids"));
		module.execute();
		assertEquals(9, getMatrixSize(module.getOperatorsMatrix().getContent().values()));
		Set<Map.Entry<String, List<String>>> operators_map = module.getOperatorsMatrix().getContent().entrySet();
		Iterator<Map.Entry<String, List<String>>> it = operators_map.iterator();
		while (it.hasNext()) {
			for (String s : it.next().getValue())
	...
	private File getFile(String s) {
		return new File(ValidationSingleReferenceTest.class.getResource("resources/" + s + ".emu").getPath());
	}
}
